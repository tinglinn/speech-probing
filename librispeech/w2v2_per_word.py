...
with h5py.File(args.output_path, 'w') as fout:
    for speaker_dir in os.scandir(args.input_path):
        for index, audio_file in enumerate(speaker_dir):
            ...
            time_stamps = os.path.join(args.time_stamp_path, speaker_id, file_id + ".csv")
            ...
            with open(time_stamps) as fd:
                ...
                for row in rd:
                    ...
                    word_count += 1
                    start_frame = int(
                        float(row[0]) * (SAMPLE_RATE / MODEL_DOWNSAMPLING))
                    end_frame = int(
                        float(row[1]) * (SAMPLE_RATE / MODEL_DOWNSAMPLING))
                    word_features = np.vstack(
                        [np.array(x)[:, start_frame:end_frame, :] for x in hidden_features])
            ...
            try:
                ...
                dset = fout.create_dataset(
                    key, (word_count, LAYER_COUNT, FEATURE_COUNT))
            except RuntimeError:
                dset = fout[key]
